...
reference = parse_tpkl("/Volumes/BAB_AGORA/CypA-S99T-Buffer-1/xray_images/CypA-S99T-Buffer-1_12_56.2us_off.tpkl".format(directories[0]))
# fig,ax = plt.subplots()
# files = []
all_vectors = []
...
for directory in directories:
	...
	for megarep in range(MEGAREPS):
		for i in range(1,REPS): # Rep 1 looks like shit
			# for temp in TEMPS:
				for index, time in enumerate(TIMES):
					try: 
						...
						print "{0}/{1}_{2}_{3}_{4}_off.tpkl".format(directory, PREFIX, megarep+1, i+1, time)
...
with open("S99T_protein.csv", 'wb') as csvfile:
