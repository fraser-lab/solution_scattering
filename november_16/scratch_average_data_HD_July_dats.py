...
# DATFILE_PREFIX = "CypA_WT_5C"
DATFILE_PREFIX = "CypA_WT_Buffer_5C"
...
length = 0
directories = argv[1:]
reference = parse_tpkl("/Volumes/lacie_share/CypA-1/xray_images/CypA-1_19_-10us-4.tpkl")
...
def combine_vectors_outliers(vectors, on_vectors, off_vectors):
	...
	if len(outlier_list) > 0:
		...
		averaged_vector, averaged_on_vector, off_vector_outlier_free = combine_vectors_outliers(new_vectors, new_on_vectors, new_off_vectors)
	return averaged_vector, averaged_on_vector, off_vectors_outlier_free
...
for time in TIMES:
	...
	averaged_vector, averaged_on_vector, off_vectors_outlier_free = combine_vectors_outliers(vectors, on_vectors[time], off_vectors[time])
	# outlier_list = chi_outliers(vectors, averaged_vector)
	# final_averaged = chi_outliers()
		# print on.q[i], np.mean(value_list), np.std(value_list)
	# plots.append()
	ax.errorbar(q, zip(*averaged_vector)[0], yerr=zip(*averaged_vector)[1], label=time)[0]
	averaged_vectors.append((time, averaged_vector))
	averaged_on_vectors.append((time, averaged_on_vector))
