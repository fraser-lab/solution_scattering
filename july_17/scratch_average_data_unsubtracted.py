        ...
        for temp in TEMPS:
            for index, time in enumerate(TIMES):
                try: 
                    ...
                    on = parse_tpkl_2("{0}/{1}_{2}_-10us{3}.tpkl".format(directory, PREFIX, i+1, off_count))
        ...
        for temp in TEMPS:
            for index, time in enumerate(TIMES):
                try:
                    ...
                    ax1.plot(off.q, off_scaled[0],c='b')
                    ...
                    # # print "appending"
                    # subtracted_vectors[time].append([(off_scaled[0][j],off_scaled[0][j]) for j in range(len(on.q))])
                except:
                    pass
                    print "one or both of the on/off pairs was tossed:"
                    print "{0}/{1}_{2}_{3}.tpkl".format(directory, PREFIX, i+1, time)
