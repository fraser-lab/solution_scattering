...
### buffer
PREFIX = "CypA-WT-1"
PKL_FILENAME = "CypA-WT-1_full_algebraic.pkl"
DATFILE_PREFIX = "CypA-WT-1"
...
length = 0
directories = argv[1:]
reference = parse.parse("/Volumes/beryllium/saxs_waxs_tjump/cypa/APS_20170302/CypA-WT-1/xray_images/CypA-WT-1_8_-10.1us.tpkl")
            ...
            for temp in TEMPS:
                for index, time in enumerate(TIMES):
                    try: 
                        ...
                        # 
                        subtracted = [(on.scaled_SA[j] - off.scaled_SA[j], np.sqrt(on.scaled_sigSA[j]**2 + off.scaled_sigSA[j]**2)) for j in range(len(on.q))]
...
def chisquared(var, ref):
    nu = len(var)-1.0
    I, sigma = list(zip(*var))
    Iref,sigmaref = list(zip(*ref))
...
def combine_vectors_outliers(vectors):
    ...
    for i in range(len(vectors[0])):
        ...
        means, stds = list(zip(*value_list))
...
for time in TIMES:
    ...
    plots.append(ax.errorbar(q, list(zip(*averaged_vector))[0], yerr=list(zip(*averaged_vector))[1], label=time)[0])
